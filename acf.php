...
class acf
{
	...
	*/
    ...
    function helpers_get_dir( $file )
    {
        ...
        $wp_plugin_dir = str_replace('\\' ,'/', plugin_dir_path( __FILE__ ));
        $dir = str_replace($wp_plugin_dir, plugins_url( '/', __FILE__ ), $dir, $count);


        if( $count < 1 )
        {
	        // if file is in wp-content folder
	        $wp_content_dir = str_replace('\\' ,'/', content_url());
